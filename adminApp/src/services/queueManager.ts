...
class RequestQueue {
  ...
  private async process() {
    if (this.activeTasks.size >= this.limit || this.queue.length === 0) return;

    const item = this.queue.shift()!;
    const activeInfo = { task: item.task, silent: item.silent };
  ...
  get state(): QueueState {
    const activeArr = Array.from(this.activeTasks);
    return {
      activeCount: this.activeTasks.size,
      queuedCount: this.queue.length,
      interactiveActiveCount: activeArr.filter((t) => !t.silent).length,
